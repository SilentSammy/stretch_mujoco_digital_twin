...
class PhysicalJointController(JointController):
    ...
    def __init__(self, robot=None, collision_mgmt=True):
        ...
        # The gripper is driven by VELOCITY like every other joint (not by the
        # bang-bang open/close position API), so that StateController's
        # proportional position control can settle it at intermediate openings.
        if self._using_stretch_gripper():
            gripper_params = RobotParams().get_params()[1]['stretch_gripper']
            self._gripper_max_vel = gripper_params['motion']['default']['vel']
            self._gripper_accel = gripper_params['motion']['max']['accel']
        ...
        self._lidar_reader = None  # Lazily started RPLidar background reader
    ...
    def set_velocities(self, vel_dict):
        ...
        # Velocity control, consistent with all other joints. The normalized
        # velocity maps linearly to the Dynamixel velocity; a strong fixed
        # acceleration is used for stopping/holding.
        if self._using_stretch_gripper():
    ...
    def stop(self):
        """Stop the robot."""
        if self._lidar_reader is not None:
            self._lidar_reader.stop()
            self._lidar_reader = None
        if self._owns_robot:
            self.robot.stop()

    def get_lidar_ranges(self):
        """Get sanitized 360-ray LiDAR range readings from the base RPLidar.

        Starts a background reader on first call.  Output matches the simulator:
        a 360-element array of distances in metres with ray 0 at the robot front
        and the index increasing counter-clockwise.  No-hit rays are np.inf.

        Returns:
            np.ndarray of length 360, or None if the LiDAR is unavailable.
        """
        if self._lidar_reader is None:
            from .lidar_driver import RPLidarReader
            try:
                reader = RPLidarReader()
                reader.start()
                reader.wait_for_first_scan(timeout=5.0)
                self._lidar_reader = reader
            except Exception as e:
                print(f"[LiDAR] Failed to start RPLidar: {e}")
                self._lidar_reader = None
                return None
        return self._lidar_reader.get_ranges()
