...
# Helper: detect if we're on a physical Stretch robot
def _on_physical_robot():
    """Check if running on an actual Stretch robot (by hostname pattern)."""
    hostname = socket.gethostname().lower()
    return 'stretch' in hostname and ('se3' in hostname or 're1' in hostname or 're2' in hostname)

# Determine which backend to use
USE_PHYSICAL = False
BACKEND_NAME = "simulation"
_stretch_body_import_failed = False
...
if os.getenv('USE_SIM', '0') == '1':
    ...
    try:
        ...
        _stretch_body_import_failed = True
        # If we're on a physical robot but can't import stretch_body, that's an environment issue
        if _on_physical_robot():
            print(f"[stretch_toolkit] WARNING: Detected physical robot (hostname: {socket.gethostname()})")
            print(f"[stretch_toolkit] WARNING: But stretch_body import failed: {e}")
            print(f"[stretch_toolkit] WARNING: You may be using the wrong Python environment.")
            print(f"[stretch_toolkit] HINT: Try activating the stretch venv or conda environment first.")
            print(f"[stretch_toolkit] HINT: Or set USE_SIM=1 if you want to force simulation mode.")
        USE_PHYSICAL = False
        BACKEND_NAME = "simulation"
...
if not USE_PHYSICAL:
    try:
        ...
        print(f"[stretch_toolkit] ERROR: Failed to import simulation dependencies: {e}")
        if _stretch_body_import_failed and _on_physical_robot():
            print(f"[stretch_toolkit] You are on a physical robot but neither backend is working.")
            print(f"[stretch_toolkit] Fix: Activate the correct Python environment with stretch_body installed.")
            print(f"[stretch_toolkit] Example: source ~/stretch_venv/bin/activate")
        else:
            print(f"[stretch_toolkit] Make sure stretch_mujoco is installed: uv sync")
        sys.exit(1)
