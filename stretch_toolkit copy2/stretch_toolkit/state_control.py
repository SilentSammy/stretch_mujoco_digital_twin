...
class StateController:
    ...
    def __init__(self, controller: JointController, desired_state: dict):
        ...
        # Backend-specific Kp overrides. The physical gripper oscillates with
        # the high sim gain, so it gets a gentler gain on hardware. Applied on
        # top of self.Kp based on the controller's backend.
        self.Kp_physical_overrides = {
            "gripper_open": 0.2,
        }
        if not getattr(self.controller, "is_simulated", False):
            self.Kp.update(self.Kp_physical_overrides)
