...
def main():
    ...
    def reach_for_object(horizontal_error, vertical_error):
        nonlocal ready_to_close, wrist_distance, wrist_center_error, wrist_authority

        if gripper_ready:
            wrist_distance = WRIST_CAMERA.get_depth(
                wrist_center,
                wrist_depth,
                sample_radius=15,
            )
            if wrist_distance is not None:
                wrist_center_error = math.hypot(horizontal_error, vertical_error)
                wrist_authority = max(0.0, 1.0 - wrist_center_error)
                command["arm_out"] = wrist_authority * KP_ARM * (
                    wrist_distance - TARGET_WRIST_DISTANCE
                )
                ready_to_close = (
                    wrist_center_error <= WRIST_CENTER_TOLERANCE
                    and abs(wrist_distance - TARGET_WRIST_DISTANCE)
    ...
    try:
        while True:
            ...
            if phase == "navigate":
                ...
                head_center = detect_object( head_rgb, head_depth, NAVIGATION_MAX_DISTANCE, HEAD_CAMERA )
                ...
                if head_center is None:
                    ...
                    track_with_head()
                    object_T = locate_object( head_center, head_depth, HEAD_CAMERA, transforms.get_cam_T(HEAD_CAMERA))
                ...
                gripper_open_at_goal = gripper_open_controller.is_at_goal()
