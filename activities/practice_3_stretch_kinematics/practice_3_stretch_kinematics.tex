...
\begin{lstlisting}[language=bash]
cp examples/teleop_boilerplate.py p3_1_camera.py
\end{lstlisting}
...
The raw depth image contains values much larger than the usual 8-bit display
range. \texttt{cv2.normalize()} maps the current frame to values from 0 to 255,
making its structure easier to see. Keep \texttt{depth\_frame} unchanged for
distance calculations; use \texttt{depth\_display} only for visualization.
...
The raw depth image stores a distance measurement at each pixel. A value of
zero usually means that no valid measurement was available. The normalized
image displayed on screen is useful for visualization, but its brightness does
not represent a fixed metric scale because each frame is normalized
independently. The public \texttt{Cameras} interface represents raw depth in
millimetres, while
