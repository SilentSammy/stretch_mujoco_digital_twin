...
\subsection*{Camera Feeds}

The teleoperation demo also provides the robot's five camera feeds. They begin
disabled to reduce simulator lag. Press \texttt{1} through \texttt{5} to toggle,
respectively, head RGB, head depth, wrist RGB, wrist depth, and wide-angle RGB.
Press \texttt{Q} or Escape to close the demo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.72\linewidth]{camera_feeds.png}
    \caption{Head RGB, head depth, wrist RGB, wrist depth, and wide-angle camera feeds.}
    \label{fig:camera_feeds}
\end{figure}
...
In \texttt{sim\_config.py}, select the RoboCasa scenario:
