...
CAMERA = HEAD_CAMERA  # Change to use that head camera.
...
def main():
    ...
    try:
        while plotter.is_open():
            ...
            success, frame, depth_frame = CAMERA.get_frames()
            camera_T = transforms.get_cam_T(CAMERA)
            object_T = None
            if success:
                center = detect_object(frame)
                if center:
                    object_T = locate_object(
                        center, depth_frame, CAMERA, camera_T
                    )
